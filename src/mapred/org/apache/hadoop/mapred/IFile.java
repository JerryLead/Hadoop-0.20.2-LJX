...
class IFile {
  ...
  public static class Writer<K extends Object, V extends Object> {
    ...
    throws IOException {
      int keyLength = key.getLength() - key.getPosition(); //第一个record的keyLength为10
      if (keyLength < 0) {
        throw new IOException("Negative key-length not allowed: " + keyLength + 
                              " for " + key);
      }
      
      int valueLength = value.getLength() - value.getPosition(); //第一个record的valueLength为4
      if (valueLength < 0) {
        throw new IOException("Negative value-length not allowed: " + 
                              valueLength + " for " + value);
      }
      ...
      out.write(key.getData(), key.getPosition(), keyLength);  //只写入length为10的key
      out.write(value.getData(), value.getPosition(), valueLength); //只写入length为4的value
  ...
  public static class Reader<K extends Object, V extends Object> {
    ...
    throws IOException {
      ...
      int keyLength = WritableUtils.readVInt(dataIn); //8
      int valueLength = WritableUtils.readVInt(dataIn); //4
      int pos = dataIn.getPosition(); //18
      ...
      final int recordLength = keyLength + valueLength; //12
      ...
      pos = dataIn.getPosition(); //18
      byte[] data = dataIn.getData(); 
      key.reset(data, pos, keyLength); //更新key
      value.reset(data, (pos + keyLength), valueLength); //更新value
