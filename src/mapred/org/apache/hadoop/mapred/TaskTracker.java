...
public class TaskTracker 
  ...
  public TaskTracker(JobConf conf) throws IOException {
    ...
    //added by LijieXu
    server.addInternalServlet("taskPerfGraph", "/taskPerfGraph", TaskPerfGraphServlet.class);
    //added end
    server.start();
  ...
  private void startResourceMonitor() {
	  if(vmstat != null)
		  return;
	  String logFileDir = System.getProperty("hadoop.log.dir");
	  final File vmstatLogDir = new File(logFileDir, "tasktrackerMetrics");
	  if(!vmstatLogDir.exists())
		  vmstatLogDir.mkdirs();
	  vmstat = new Thread() {
		
  		private String shellcommand = "exec '/usr/bin/vmstat' -n 2 >> " + 
  					vmstatLogDir.getAbsolutePath() + File.separator + "vmstat.log";
