...
class MapTask extends Task {
  ...
  implements MapOutputCollector<K, V>, IndexedSortable {
    ...
    public synchronized void flush() throws IOException, ClassNotFoundException, //Map Task所有的K,V都已经输出时调用，清空buffer，merge所有的spill
                                            InterruptedException {               //成为spill.out和spill.index.out，在merge中可以进行combine
    ...
    private void sortAndSpill(boolean isFlush) throws IOException, ClassNotFoundException, //本方法完成spill0.out, spill1.out,...的产生，还有index的产生。
                                       InterruptedException {
      ...
      long size = (bufend >= bufstart
          ...
          : (bufvoid - bufend) + bufstart) +
                  partitions * APPROX_HEADER_LENGTH;  //估算要spill的字节数
      */
      long size = (bufend >= bufstart
  	          ? bufend - bufstart
      ...
      try {
        // create spill file
        final SpillRecord spillRec = new SpillRecord(partitions); //记录spill的index，里面刚好存放partition个（startOffset,rawLength,partLength)
        final Path filename = mapOutputFile.getSpillFileForWrite(getTaskID(),
            numSpills, size); //opt/hadooptmp/mapred/local/taskTracker/jobcache/job_201207042316_0001/attempt_201207042316_0001_m_000002_0/output/spill0.out
        out = rfs.create(filename);

        final int endPosition = (kvend > kvstart)
          ? kvend
          : kvoffsets.length + kvend;  //将endPosition变成绝对的（从0到无穷大）
        sorter.sort(MapOutputBuffer.this, kvstart, endPosition, reporter); //排序kvoffsets，注意endPosition）
        int spindex = kvstart; //spill的开始位置，是kvoffsets中的kvstart
        IndexRecord rec = new IndexRecord();  //记录每个spill中每个partition的偏移量等信息
        InMemValBytes value = new InMemValBytes(); //将从buffer中读取的value先放入mem中
        ...
        for (int i = 0; i < partitions; ++i) {
          ...
          try {
            ...
            writer = new Writer<K, V>(job, out, keyClass, valClass, codec,
                                      spilledRecordsCounter); //spilled records的计数器
            
            
            if (combinerRunner == null) { //如果没有combiner，直接从buffer中读取排序好的K，V输出到HDFS
              // spill directly
              DataInputBuffer key = new DataInputBuffer();
              while (spindex < endPosition &&   //把kvoffsets中partition为同一个reducer的所有<K,V>过滤出来，kvoffsets已经排序好
                  kvindices[kvoffsets[spindex % kvoffsets.length]
                            + PARTITION] == i) {
                final int kvoff = kvoffsets[spindex % kvoffsets.length]; //对应spindex指向kvindices
                getVBytesForOffset(kvoff, value); //将当前spindex指向的K,V中的V读取出来放到内存
                key.reset(kvbuffer, kvindices[kvoff + KEYSTART], //读取key，key一定是连续的，不存在跨buffer，由buffer的reset方法控制，引入bufvoid
              ...
              if (spstart != spindex) {
                ...
                combinerRunner.combine(kvIter, combineCollector); //如果有combiner，就使用kvIter来读取K，V，进行combine后再输出。
              }
            }

            // close the writer
            writer.close(); //如果没有combiner，此时只写入了某个spill中的partition i部分的所有K，V records

            // record offsets
            rec.startOffset = segmentStart;  //segmentStart记录了该partition对应在spill0.out的开始位置
            rec.rawLength = writer.getRawLength();  //rawLength记录了该partition对应在spill0.out的长度
            rec.partLength = writer.getCompressedLength(); //partLength记录了该partition对应在spill0.out的压缩长度
        ...
        if (totalIndexCacheMemory >= INDEX_CACHE_MEMORY_LIMIT) {
          ...
          spillRec.writeToFile(indexFilename, job); //如果超过了存放index的cache限制就输出index
        } else {
    ...
    private void getVBytesForOffset(int kvoff, InMemValBytes vbytes) {
      final int nextindex = (kvoff / ACCTSIZE ==  //kvend总是与kvindex一致，kvindex总是指向要写入的点，nextindex指向下一个value在buffer的位置
                            (kvend - 1 + kvoffsets.length) % kvoffsets.length)
        ? bufend //如果没有下一个key，那么使用bufend-当前value的开始位置来得到vallen
        : kvindices[(kvoff + ACCTSIZE + KEYSTART) % kvindices.length]; //通过使用kvindices中下一个key的开始位置-当前value的开始位置来得到vallen
      int vallen = (nextindex >= kvindices[kvoff + VALSTART]) //通过计算当前value在buffer中的索引和下一个value在buffer中的索引的差来得到value的长度
        ? nextindex - kvindices[kvoff + VALSTART] 
        : (bufvoid - kvindices[kvoff + VALSTART]) + nextindex; //此时是最后一个key在buffer最右端，value跨两头或者在buffer左端
    ...
    protected class MRResultIterator implements RawKeyValueIterator {
      ...
      public DataInputBuffer getValue() throws IOException {
        getVBytesForOffset(kvoffsets[current % kvoffsets.length], vbytes); //重用没有combine时候的getValue方法
        return vbytes;
      }
    ...
    private void mergeParts() throws IOException, InterruptedException, 
      ...
      for(int i = 0; i < numSpills; i++) {
        ...
        finalOutFileSize += rfs.getFileStatus(filename[i]).getLen(); //计算spill0.out, spill{i}.out的长度总和
      }
      if (numSpills == 1) { //the spill is the final output 直接重命名spill0.out为file.out就行
        rfs.rename(filename[0],
            new Path(filename[0].getParent(), "file.out"));
        if (indexCacheList.size() == 0) {  //如果只有spill{i}.index.out只有一个，直接重命名为file.out.index就行
          rfs.rename(mapOutputFile.getSpillIndexFile(mapId, 0),
              new Path(filename[0].getParent(),"file.out.index"));
        } else {
          indexCacheList.get(0).writeToFile( //如果由于index的cache大小限制，生成了多个spill{i}.index，那么直接append到file.out.index就行
      ...
      for (int i = indexCacheList.size(); i < numSpills; ++i) { //将所有的index（存放在硬盘上的spill{i}.index）都重新读到indexCacheList中
      ...
      if (numSpills == 0) { //如果map没有输出，那么还是要创建file.out.index，不过里面写入的内容没有意义
        //create dummy files
        IndexRecord rec = new IndexRecord(); 
      ...
      }
      {
        ...
        for (int parts = 0; parts < partitions; parts++) {
          ...
          List<Segment<K,V>> segmentList =
            new ArrayList<Segment<K, V>>(numSpills);  //每一个part有一个segmentList
          ...
          //added end
          for(int i = 0; i < numSpills; i++) {
            IndexRecord indexRecord = indexCacheList.get(i).getIndex(parts); //从spill{i}.index读出该part对应的indexRecord

            Segment<K,V> s =
              new Segment<K,V>(job, rfs, filename[i], indexRecord.startOffset, //构造Segment，指向spill{i}.out其中对应part部分的索引位置
            ...
            //added end
            if (LOG.isDebugEnabled()) {  //如果想看到详细列表，那么打开debug选项即可
          ...
          //merge
          @SuppressWarnings("unchecked")
          RawKeyValueIterator kvIter = Merger.merge(job, rfs, //真正的merge开始，将所有属于该part的segment merge成一个大的，merge需要进行part次
                         keyClass, valClass, codec,
                         segmentList, job.getInt("io.sort.factor", 100), //为啥不用segmentList的length作为factor?
                         new Path(mapId.toString()),
                         job.getOutputKeyComparator(), reporter,
                         null, spilledRecordsCounter); //readsCounters为null，writeCounters为spilled records counter
          ...
          Writer<K, V> writer =
              new Writer<K, V>(job, finalOut, keyClass, valClass, codec,
                               spilledRecordsCounter);
          if (combinerRunner == null || numSpills < minSpillsForCombine) { //merge以后，如果没有combine，或者spill{i}.out个数太少就不做combine，直接输出
            Merger.writeFile(kvIter, writer, reporter, job); 
          } else {  //否则先进行combine再进行输出
            combineCollector.setWriter(writer);
            combinerRunner.combine(kvIter, combineCollector);
          }
         
          //此时已经完成将所有part为i的intermediate.{j}的合并添加到file.out
          //close
          writer.close();
          ...
          LOG.info("[AfterMergeAndCombine][Partition " + parts + "]<RecordsBeforeCombine = " 
        		  			...
        		  			+ "CompressedLength = " + rec.partLength + ">");
          //added end
        } //彻底将所有的part对应的k,v合并完输出为file.out
        spillRec.writeToFile(finalIndexFile, job); //完成file.index.out的生成
        
        
        finalOut.close();
        for(int i = 0; i < numSpills; i++) {
          rfs.delete(filename[i],true);   //删除spill{i}.out
        }
      }
    }
